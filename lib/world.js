...
// World.prototype.updateBoost = function(prawn)  {
//   var player2boost = document.querySelector('.player2-boost');
//   var player1boost = document.querySelector('.player1-boost');
//   player2boost.innerText = 'BOOST B \n' + this.boost[1];
//   player1boost.innerText = 'BOOST A \n' + this.boost[0];
//   }
// };

// world.prototype.updateSpeed(prawn) {
//   // if someone presses this and their speed is already 3.5 you ignore
//
//   if (prawn === this.prawns[0]) {
//     this.speep[1] = this.score[1] + 1;
//     player2score.innerText = 'PRAWN B \n' + this.score[1];
//   }
//   if (prawn === this.prawns[1]) {
//     this.score[0] = this.score[0] + 1;
...
//
//
//   this.prawns(prawn).speed = 3.5;
//     window.setTimeout(function() {}, 500);
//   this.prawns(prawn).speed = 3;
// }

World.prototype.boostPrawnA = function (playerNumber) {
  _self = this;
  var boosts = this.boosts[0];
  if (this.boosts[0] === 0) {
    return console.log('oops');
  } else {
    this.prawns[0].speed = 1;
    this.prawns[0].color = '#F1AA49';
    var boosts = boosts - 1;
    this.boosts[0] = boosts;
      window.setTimeout(function()
      {
        _self.prawns[0].color = '#B0171F';
        _self.prawns[0].speed = 0.5 ;
      }, 500);
    var player1boost = document.querySelector('.player1-boost');
    player1boost.innerText = 'BOOST A \n' + boosts;
  }
...
World.prototype.boostPrawnB = function (playerNumber) {
  _self = this;
  var boosts = this.boosts[1];
  if (this.boosts[1] === 0) {
    return console.log('oops');
  } else {
    this.prawns[1].speed = 1;
    this.prawns[1].color = '#F1AA49';
    var boosts = boosts - 1;

    this.boosts[1] = boosts;
      window.setTimeout(function()
      {
        _self.prawns[1].color = '#F5D3E2';
        _self.prawns[1].speed = 0.5 ;
      }, 500);
    var player2boost = document.querySelector('.player2-boost');
    player2boost.innerText = 'BOOST B \n' + boosts;
  }
