...
Prawn.prototype.checkPoopUp = function() {
  for (i=0; i<this.pastLocations.length; i++) {
    var nextLocation = {x: this.x, y: this.y - 1};
    if (this.pastLocations[i].x === nextLocation.x && this.pastLocations[i].y === nextLocation.y);
...
// Prawn.prototype.checkUp = function() {
//   var nextLocation = {x: this.x, y: this.y - 1};
//   debugger;
//   if (this.pastLocations.includes(nextLocation)) {alert('game over, Jhun')}
// };


Prawn.prototype.checkDown = function() {
  if (this.dir === 'left' || this.dir === 'right') this.moveDown();
};
